...
pub fn cartesian_product<const K:usize, T:Copy>(iters:[Vec<T>;K]) -> Vec<Vec<T>> {
...
/// create the sorted cartesian product of N orderable vectors
pub fn sorted_cartesian_product<const K:usize, T:Copy+Ord>(iters:[Vec<T>;K]) -> Vec<Vec<T>> {
    let mut output = cartesian_product(iters);
    output.sort();
    output
}