...
impl<T:Float> Matrix<T, 2> {
    ...
    pub fn inverse(&self, method:InverseMethod) -> Result<Matrix<T, 2>, MatrixError<2>> {
    ...
    pub fn solve(self) -> Result<Matrix<T, 1>, MatrixError<2>> {
    ...
    pub fn gauss_jordan_inverse(&self) -> Result<Matrix<T, 2>, MatrixError<2>> {
        if self.shape[0] != self.shape[1] {
            Err(MatrixError::InvalidShape(self.shape))
        } else {


            let identity = Matrix::<T, 2>::identity(self.shape[0]);
